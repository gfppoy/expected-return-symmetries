...
def load_permutation(path, i):
    with open(f'{path}symmetry_{i}_in_3p.pkl', 'rb') as file:
        in_permutation = pickle.load(file)
    with open(f'{path}symmetry_{i}_out_3p.pkl', 'rb') as file:
...
@hydra.main(version_base=None, config_path="config", config_name="real_op_ippo_ff_hanabi")
def main(config):
    ...
    in_permutations = jnp.zeros((120, 956, 956))
    out_permutations = jnp.zeros((120, 31, 31))
    for i in range(120):
