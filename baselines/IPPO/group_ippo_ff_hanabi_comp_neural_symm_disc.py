...
def selective_zero_grads(grads, freeze_layers=('Dense_0', 'Dense_1', 'Dense_2', 'Dense_7', 'Dense_8')):
    ...
    def zero_out_grads(layer_name, grad):
        ...
        """
        # Check if layer should be frozen (1 for freeze, 0 for not).
...
class ActorCritic(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        obs, dones, avail_actions, num_compositions, third_in_permutation_layer_1_kernel, third_in_permutation_layer_2_kernel, third_in_permutation_layer_3_kernel, third_in_permutation_layer_1_bias, third_in_permutation_layer_2_bias, third_in_permutation_layer_3_bias, third_out_permutation, first_out_permutation = x
        n = 658
        ...
        def apply_in_permutation(x):
            x = nn.relu(x @ third_in_permutation_layer_1_kernel + third_in_permutation_layer_1_bias)
            x = nn.relu(x @ third_in_permutation_layer_2_kernel + third_in_permutation_layer_2_bias)
            x = nn.relu(x @ third_in_permutation_layer_3_kernel + third_in_permutation_layer_3_bias)
            return x
        ...
        def apply_third_out_permutation(x):
            return x @ third_out_permutation
        transformed_actor_mean = jax.lax.cond(num_compositions == 2, apply_third_out_permutation, lambda x: x, actor_mean)
        out_permutation = nn.Dense(self.action_dim, kernel_init=permute_two_indices_init(index1=11, index2=13), use_bias=False)
        transformed_actor_mean = out_permutation(transformed_actor_mean)
        def apply_first_out_permutation(x):
            return x @ first_out_permutation
        transformed_actor_mean = jax.lax.cond(num_compositions == 2, apply_first_out_permutation, lambda x: x, transformed_actor_mean)
...
def make_train(config):
    ...
    in_permutations_layer_1_kernel, 
    in_permutations_layer_2_kernel,
    in_permutations_layer_3_kernel,
    in_permutations_layer_1_bias,
    in_permutations_layer_2_bias,
    in_permutations_layer_3_bias,
    out_permutations,
    symm_so_far):
        ...
        init_x = (
            ...
            jnp.zeros((658)),
            jnp.zeros((658)),
            jnp.zeros((env.action_space(env.agents[0]).n, env.action_space(env.agents[0]).n)),
            jnp.zeros((env.action_space(env.agents[0]).n, env.action_space(env.agents[0]).n))
        )
        network_params = []
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                train_state, env_state, last_obs, last_done, rng, num_compositions, perm_indices = runner_state
                ...
                def apply_obs_transform(x):
                    x = nn.relu(x @ in_permutations_layer_1_kernel[perm_indices[0]] + in_permutations_layer_1_bias[perm_indices[0]])
                    x = nn.relu(x @ in_permutations_layer_2_kernel[perm_indices[0]] + in_permutations_layer_2_bias[perm_indices[0]])
                    x = nn.relu(x @ in_permutations_layer_3_kernel[perm_indices[0]] + in_permutations_layer_3_bias[perm_indices[0]])
                    return x

                obs_batch = jax.lax.cond(num_compositions == 2, apply_obs_transform, lambda x: x, obs_batch)

                ac_in = (obs_batch[np.newaxis, :], last_done[np.newaxis, :], avail_actions[np.newaxis, :], num_compositions, in_permutations_layer_1_kernel[perm_indices[1]], in_permutations_layer_2_kernel[perm_indices[1]], in_permutations_layer_3_kernel[perm_indices[1]], in_permutations_layer_1_bias[perm_indices[1]], in_permutations_layer_2_bias[perm_indices[1]], in_permutations_layer_3_bias[perm_indices[1]], out_permutations[perm_indices[1]], out_permutations[perm_indices[0]])
                ...
                runner_state = [train_state, env_state, obsv, done_batch, rng, num_compositions, perm_indices]
            ...
            permutation_indices = jax.random.choice(_rng, in_permutations_layer_1_kernel.shape[0], shape=(config["NUM_PRETRAINED"],2), replace=True)
            for i in range(config["NUM_PRETRAINED"]):
                ...
                _, _rng = jax.random.split(runner_state[4])
                probability = 0.35 #0.05 + 0.2 * jnp.clip(update_steps / 1000, 0, 1)
                ...
                rs.append(permutation_indices[i])
            ...
            for i, rs in enumerate(runner_states):
                ...
                train_state, env_state, last_obs, last_done, rng, num_compositions, perm_indices = rs
                if i > 0:
                ...
                last_obs_batch = jax.lax.cond(num_compositions == 2, apply_obs_transform, lambda x: x, last_obs_batch)

                avail_actions = jnp.ones(
                    (config["NUM_ACTORS"] // config["NUM_PRETRAINED"], env.action_space(env.agents[0]).n)
                )
                ac_in = (last_obs_batch[np.newaxis, :], last_done[np.newaxis, :], avail_actions, num_compositions, in_permutations_layer_1_kernel[perm_indices[1]], in_permutations_layer_2_kernel[perm_indices[1]], in_permutations_layer_3_kernel[perm_indices[1]], in_permutations_layer_1_bias[perm_indices[1]], in_permutations_layer_2_bias[perm_indices[1]], in_permutations_layer_3_bias[perm_indices[1]], out_permutations[perm_indices[1]], out_permutations[perm_indices[0]])
                    ...
                    def _update_minbatch(train_state, batch_info):
                        ...
                        def _loss_fn(params, traj_batch, gae, targets):
                            ...
                            pi, value = network.apply(params,
                                                    (traj_batch.obs, traj_batch.done, traj_batch.avail_actions, num_compositions, in_permutations_layer_1_kernel[perm_indices[1]], in_permutations_layer_2_kernel[perm_indices[1]], in_permutations_layer_3_kernel[perm_indices[1]], in_permutations_layer_1_bias[perm_indices[1]], in_permutations_layer_2_bias[perm_indices[1]], in_permutations_layer_3_bias[perm_indices[1]], out_permutations[perm_indices[1]], out_permutations[perm_indices[0]]))
                            ...
                            # INVERTIBILITY
                            transformed_obs = nn.relu(jnp.dot(nn.relu(jnp.dot(nn.relu(jnp.dot(traj_batch.obs, params['params']['Dense_0']['kernel']) + params['params']['Dense_0']['bias']), params['params']['Dense_1']['kernel']) + params['params']['Dense_1']['bias']), params['params']['Dense_2']['kernel']) + params['params']['Dense_2']['bias'])
                            transformed_obs = nn.relu(jnp.dot(nn.relu(jnp.dot(nn.relu(jnp.dot(transformed_obs, params['params']['Dense_0']['kernel']) + params['params']['Dense_0']['bias']), params['params']['Dense_1']['kernel']) + params['params']['Dense_1']['bias']), params['params']['Dense_2']['kernel']) + params['params']['Dense_2']['bias'])
                            l1_transformed_obs_identity = jnp.sum(jnp.abs(transformed_obs - traj_batch.obs))

                            total_loss = (
                                    loss_actor
                                    + config["VF_COEF"] * value_loss
                                    - config["ENT_COEF"] * entropy
                                    + config["IDENTITY_COEFF"] * l1_transformed_obs_identity
                            )
...
@hydra.main(version_base=None, config_path="config", config_name="group_ippo_ff_hanabi_comp_neural_symm_disc")
def main(config):
    ...
    config["NUM_PERMUTATIONS"] = len(config["LOAD_PERMS"].split(',')) + 1

    in_permutations_layer_1_kernel = jnp.zeros((config["NUM_PERMUTATIONS"], 658, 658))
    in_permutations_layer_2_kernel = jnp.zeros((config["NUM_PERMUTATIONS"], 658, 658))
    in_permutations_layer_3_kernel = jnp.zeros((config["NUM_PERMUTATIONS"], 658, 658))
    in_permutations_layer_1_bias = jnp.zeros((config["NUM_PERMUTATIONS"], 658))
    in_permutations_layer_2_bias = jnp.zeros((config["NUM_PERMUTATIONS"], 658))
    in_permutations_layer_3_bias = jnp.zeros((config["NUM_PERMUTATIONS"], 658))
    out_permutations = jnp.zeros((config["NUM_PERMUTATIONS"], 21, 21))

    for i, permutation_path in enumerate(config["LOAD_PERMS"].split(',')):
        ...
        in_permutations_layer_1_kernel = in_permutations_layer_1_kernel.at[i, :].set(layer_1_kernel)
        in_permutations_layer_2_kernel = in_permutations_layer_2_kernel.at[i, :].set(layer_2_kernel)
        in_permutations_layer_3_kernel = in_permutations_layer_3_kernel.at[i, :].set(layer_3_kernel)

        in_permutations_layer_1_bias = in_permutations_layer_1_bias.at[i, :].set(layer_1_bias)
        in_permutations_layer_2_bias = in_permutations_layer_2_bias.at[i, :].set(layer_2_bias)
        in_permutations_layer_3_bias = in_permutations_layer_3_bias.at[i, :].set(layer_3_bias)

        out_permutations = out_permutations.at[i, :].set(out_permutation)

    in_permutations_layer_1_kernel = in_permutations_layer_1_kernel.at[-1, :].set(jnp.eye(658))
    in_permutations_layer_2_kernel = in_permutations_layer_2_kernel.at[-1, :].set(jnp.eye(658))
    in_permutations_layer_3_kernel = in_permutations_layer_3_kernel.at[-1, :].set(jnp.eye(658))

    in_permutations_layer_1_bias = in_permutations_layer_1_bias.at[-1, :].set(jnp.zeros(658))
    in_permutations_layer_2_bias = in_permutations_layer_2_bias.at[-1, :].set(jnp.zeros(658))
    in_permutations_layer_3_bias = in_permutations_layer_3_bias.at[-1, :].set(jnp.zeros(658))

    out_permutations = out_permutations.at[-1, :].set(jnp.eye(21))
