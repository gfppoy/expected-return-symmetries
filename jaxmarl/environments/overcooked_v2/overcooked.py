...
class OvercookedV2(MultiAgentEnv):
    ...
    def step_agents(
        ...
        def _compute_swapped_agents(original_positions, new_positions):
            ...
            # swap_mask = jnp.fill_diagonal(swap_mask, False, inplace=False) # jaxlib 0.4.28
            n_agents = swap_mask.shape[0]
            swap_mask = swap_mask.at[jnp.arange(n_agents), jnp.arange(n_agents)].set(False)
        ...
        def _timestep_wrapper(cell):
            ...
            def _indicator(cell):
                # new_extra = jnp.clip(cell[2] - 1, min=0) # jaxlib 0.4.28
                new_extra = jnp.maximum(cell[2] - 1, 0)
