...
def make_train(config):
    ...
    out_permutations):
        ...
        shuffle_colour_indices = jax.random.choice(__rng, in_permutations_layer_1_kernel.shape[0], shape=(config["NUM_ENVS"],), replace=True)
        init_x = (
            jnp.zeros(
        ...
        shuffle_colour_indices = jax.random.choice(__rng, in_permutations_layer_1_kernel.shape[0], shape=(config["NUM_ACTORS"],), replace=True)

        def transform_obs(obs, layer_1_kernel, layer_1_bias, layer_2_kernel, layer_2_bias, layer_3_kernel, layer_3_bias):
            # Layer 1
            hidden_obs1 = jnp.dot(obs, layer_1_kernel) + layer_1_bias
            hidden_obs1 = nn.relu(hidden_obs1)

            # Layer 2
            hidden_obs2 = jnp.dot(hidden_obs1, layer_2_kernel) + layer_2_bias
            hidden_obs2 = nn.relu(hidden_obs2)

            # Layer 3
            transformed_obs = jnp.dot(hidden_obs2, layer_3_kernel) + layer_3_bias
            transformed_obs = nn.relu(transformed_obs)
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                obs_batch = jax.vmap(transform_obs, in_axes=(0, 0, 0, 0, 0, 0, 0))(
                    obs_batch.reshape(-1, obs_batch.shape[-1]),
                    in_permutations_layer_1_kernel[shuffle_colour_indices],
                    in_permutations_layer_1_bias[shuffle_colour_indices],
                    in_permutations_layer_2_kernel[shuffle_colour_indices],
                    in_permutations_layer_2_bias[shuffle_colour_indices],
                    in_permutations_layer_3_kernel[shuffle_colour_indices],
                    in_permutations_layer_3_bias[shuffle_colour_indices],
                ).reshape(obs_batch.shape)

                ac_in = (obs_batch[np.newaxis, :], 
                ...
                rng, updated_shuffle_colour_indices = _resample_indices(rng, shuffle_colour_indices, done_batch, in_permutations_layer_1_kernel.shape[0])
            ...
            last_obs_batch = jax.vmap(transform_obs, in_axes=(0, 0, 0, 0, 0, 0, 0))(
                ...
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        pi, value = network.apply(params,
                                                  ...
                                                  out_permutations[traj_batch.shuffle_colour_indices.ravel()]))
...
@hydra.main(version_base=None, config_path="config", config_name="neural_op_ippo_ff_hanabi")
def main(config):
